...
async def serve_static(path: str, request: Request):
    ...
    html_path = STATIC_DIR / f"{normalized_path}.html"
    if path.endswith("/") and normalized_path and html_path.is_file():
        query = f"?{request.url.query}" if request.url.query else ""
        return RedirectResponse(url=f"/{normalized_path}{query}", status_code=308)
