# Privacy Router 통합 연구 지도 Excalidraw 설계 — 정보 누락 없는 쉬운 설명판

## 목적

기존의 `논문 이야기`, `시스템 파이프라인`, `Claim–Evidence` 보드 3개를 하나의 Excalidraw 세션으로 통합한다. 원본의 텍스트 요소 106개(논문 이야기 28개, 시스템 파이프라인 23개, Claim–Evidence 55개)를 빠짐없이 반영한다. 처음 보는 교수·심사자는 전문 용어를 별도로 찾지 않고도 연구 문제와 근거 한계를 이해하고, 연구팀은 같은 보드에서 구현 구조·실험 통과 조건·다음 행동을 찾을 수 있어야 한다.

## 확정 사항

- 독자: 교수·심사자와 연구팀을 함께 고려한 균형형
- 구조: 위에서 아래로 읽는 단일 세로형 캔버스
- 언어: 쉬운 한국어를 먼저 쓰고, 꼭 필요한 표준 영문 용어는 첫 등장에만 괄호로 병기
- 설명 수준: 모든 주요 카드를 `무엇인가 → 왜 필요한가 → 어떻게 확인하는가`의 완전한 문장으로 작성
- 상태 문구: `GO_TO_PILOT (8/10) — 논문 결과가 확립된 상태는 아님`
- 근거 원칙: 계획·가설·단위 테스트 결과를 논문 근거로 표현하지 않음
- 기존 3개 `.excalidraw` 파일은 보존하고 통합 파일만 교체

## 완전성·문장 원칙

- 원본의 제목, 부제목, 상태, 본문 카드, 읽는 순서, 범례, 출처를 모두 포함한다. 반복된 내용도 의미가 다르면 삭제하지 않는다.
- 원본 텍스트 요소마다 새 보드의 대응 요소를 기록한다. 자동 검증에서 `106/106`이 확인되지 않으면 완료로 보지 않는다.
- 명사만 나열하지 않는다. 각 문장은 주어와 서술어를 갖추고, 한 문장에는 한 가지 판단만 담는다.
- 영어와 한국어를 한 구절 안에서 임의로 섞지 않는다. 표준 영문 용어가 필요하면 `쉬운 한국어(영문 용어)` 형식으로 한 번 정의한 뒤 한국어 표현을 사용한다.
- 모든 수치, 실험 번호, 현재 상태, 실패 시 범위 축소안은 원본과 동일하게 유지한다.
- 약어는 처음 등장할 때 풀어 쓴다. 예: `안전 입력 오탐률(safe-FPR)`, `곡선 아래 위험–범위 면적(AURC)`.
...
캔버스는 약 2000px 폭의 세로형 연구 지도로 구성한다. 전체 내용을 한 화면에 억지로 축소하지 않는다. 공유 URL을 열면 상단 개요가 먼저 보이고, 사용자는 아래로 스크롤하여 각 장을 읽는다. 각 장의 시작에는 번호, 제목, 이 장에서 답하는 질문을 둔다.
...
3. 시스템 파이프라인
4. 논문 주장과 근거
5. 연구 단계별 통과 조건과 다음 행동
6. 용어 안내와 출처

본문 글꼴은 18px 이상, 보조 설명은 16px 이상, 장 제목은 26–30px, 전체 제목은 34px 이상으로 유지한다. 카드 한 줄이 길어지면 글꼴을 줄이지 않고 줄바꿈이나 카드 폭·높이를 조정한다.
...
Overview 아래에는 뒤의 다섯 장으로 이동하는 목차 카드를 배치한다. 목차 카드는 시각적 탐색 표지이며 새로운 주장이나 결과를 담지 않는다.
...
보드에서 사용한 전문 용어를 다음과 같이 쉬운 말로 정의한다.

- `외부 전송 후보(candidate egress)`: 모델이나 도구 제공자에게 보내기 직전의 내용
- `원문 구간(exact span)`: 민감 정보가 차지하는 정확한 문자 범위
- `사실 조건(factual factors)`: 공개 상태, 수신자 권한, 사용 목적 권한, 원문 값 필요성
- `고정 정책 P0(frozen policy P0)`: 입력 조건에 따라 허용·가림·로컬 처리·사용자 질문·거부 중 하나를 고르는 사전 고정 규칙
- `실제 전송 내용(serialized egress bytes)`: 네트워크 요청으로 변환되어 외부에 실제로 전달되는 바이트
- `신뢰 경계(trust boundary)`: 이 선을 넘으면 정보가 외부 시스템의 통제 아래 놓이는 경계
- `오탐률(false-positive rate)`: 안전한 내용을 민감하다고 잘못 판단한 비율
- `신뢰구간(confidence interval)`: 반복 표본에서 실제 효과가 있을 범위를 추정한 구간
- `비열등성(non-inferiority)`: 새 방법이 기준 방법보다 정해진 허용치 이상 나빠지지 않았다는 조건
- `실패 시 범위 축소안(failure fallback)`: 통과 기준을 못 맞췄을 때 논문 주장을 줄이는 방법

각 장 하단에는 원본 보드 이름과 근거 문서를 표시한다. 용어 안내는 설명을 위한 것이며 새로운 주장이나 지표를 만들지 않는다.
...
- 하나의 `.excalidraw` 파일과 공유 URL에서 여섯 섹션을 모두 확인할 수 있음
- 기존 3개 보드는 변경되지 않음
- 원본 텍스트 요소 106개가 새 보드 요소에 모두 대응하며 자동 검증 결과가 `106/106`, 누락 `0`임
- 제목·부제목·상태·본문·읽는 순서·범례·출처가 모두 포함됨
- 전체 개요에서 `GO_TO_PILOT`와 `논문 결과가 확립된 상태는 아님`이 즉시 보임
- 논문 이야기의 각 단계에 핵심 내용, 배경, 검증 방법, 다음 단계와의 연결이 포함됨
- 시스템 파이프라인의 각 단계에 입력, 처리, 출력, 실패 시 동작이 포함됨
- C1–C7 각각에 쉬운 질문, 실험, 통과 기준의 뜻, 현재 근거 상태, 실패 시 범위 축소안이 포함됨
- G0–G6 각각에 목적, 통과 조건, 필요한 산출물, 지금 할 일이 포함됨
- 전문 용어는 쉬운 한국어로 먼저 설명하고 필요한 경우에만 첫 등장에 영문을 병기함
- 명사 나열이나 조사 누락 없이 완전한 문장으로 작성됨
- 본문 글꼴은 18px 이상, 보조 설명은 16px 이상이며 카드 겹침·텍스트 잘림·의미 없는 교차선이 없음
- 공유 URL을 실제 브라우저에서 열고 개요와 각 장을 스크롤하여 시각 검증함
