						...
						<Card>
							<form class="space-y-5 p-5 sm:p-6" onsubmit={(event) => { event.preventDefault(); void handleCreateModel(); }}>
								...
								<div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
									...
									<label class="space-y-1.5 text-xs font-medium text-slate-300"><span>{$t('admin.models.display_name')}</span><input bind:value={modelDraft.display_name} placeholder="모델 이름" class="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white placeholder:text-slate-400 focus:border-blue-500 focus:outline-none" /></label>
