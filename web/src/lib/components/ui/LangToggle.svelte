...
<button
	...
	class={compact
		...
		: 'cursor-pointer rounded-lg border border-slate-700 bg-slate-800 px-3 py-1.5 text-xs text-slate-300 transition hover:bg-slate-700 hover:text-white'}
>
	{compact ? ($locale === 'en' ? 'KO' : 'EN') : $locale === 'en' ? '🇰🇷 한국어' : '🇺🇸 English'}
</button>
