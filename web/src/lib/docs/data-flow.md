...
See [Query Aggregation Spec](/docs/query-aggregation) for the normative schema, formulas, and fail-closed acceptance tests.
...
Caller-facing API/MCP metadata may include sensitive span details because the caller supplied the original prompt, but this metadata must still be treated as sensitive operational data.
...
|---|---|---|
| Raw input prompt | Process memory | Request lifetime unless caller manages it. |
| Extraction records | Memory/cache; sensitive if persisted | Session/cache lifetime. |
