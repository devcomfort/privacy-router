...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\linewidth]{../assets/generated/context-sensitive-detection.png}
  \caption{Conceptual illustration of context-sensitive information. Research, business, and personal prompts can require different protections even when their surface patterns look similar. This image is explanatory only; evaluation results are reported later.}
  \label{fig:context-sensitive-detection}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\linewidth]{../assets/generated/two-tier-routing.png}
  \caption{Conceptual illustration of the two-tier routing boundary. The router sends masked, non-essential requests to an external model and retains requests that require sensitive context for local processing.}
  \label{fig:two-tier-routing}
\end{figure}
...
Privacy Router uses a single-model configuration for the Extractor, with a rule-based Judge that requires no LLM calls:
...
\text{Judge cost} &= \$0 \text{ (rule-based, no LLM call)} \\
...
% ===========================================================================
\section{Smartening: Extractor Architecture}
% ===========================================================================

\subsection{ExtractorCore (Socratic Extraction)}

An SLM analyzes user text to detect sensitive information using Socratic reasoning. The model asks three questions per sentence to derive free-form SCREAMING\_CASE categories:

\begin{verbatim}
Q1: What is the core attribute of this information?
Q2: Why is this information sensitive?
Q3: How would I name this in one word?
-> SCREAMING_SNAKE_CASE category
\end{verbatim}

\subsection{Critic (Post-Review, precision="high")}

When \texttt{precision="high"}, a second SLM reviews ExtractorCore results and supplements missing records. The Critic runs on any non-empty text, regardless of ExtractorCore results. During merging, the following validations are performed:
...
\begin{itemize}[nosep]
  ...
  \item Same validation bar as ExtractorCore (confidence $\geq$ 0.5, SCREAMING\_CASE regex)
\end{itemize}

\subsection{Judge (Rule-Based)}

The Judge determines policy actions based on \texttt{is\_essential} flags without LLM calls:

\begin{itemize}[nosep]
  \item \texttt{is\_essential > 0} $\rightarrow$ \texttt{route\_to\_local} (sensitive data is the query itself)
  \item All maskable $\rightarrow$ \texttt{mask\_and\_send} (replace spans with placeholders)
  \item Not sensitive $\rightarrow$ \texttt{route\_to\_external} (pass through unchanged)
\end{itemize}
...
\begin{enumerate}[nosep]
  ...
  \item The rule-based Judge configuration (no LLM call) achieved an operating cost of approximately \$0.075/month.
\end{enumerate}
