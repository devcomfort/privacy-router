...
\begin{figure}[htbp]
  ...
  \caption{문맥 기반 민감 정보의 개념도. 연구·사업·개인 프롬프트는 표면적 형식이 유사해도 서로 다른 보호가 필요할 수 있다. 본 이미지는 설명용이며, 평가 결과는 뒤에서 제시한다.}
...
\begin{figure}[htbp]
  ...
  \caption{Two-Tier 라우팅 경계의 개념도. Router는 마스킹된 비핵심 요청을 외부 모델로 보내고, 민감 문맥이 필요한 요청은 로컬 처리 경로에 유지한다.}
...
Privacy Router는 Extractor에 단일 모델을 사용하고, Judge는 LLM 호출 없이 규칙 기반으로 동작한다:
...
\text{Judge 비용} &= \$0 \text{ (규칙 기반, LLM 미사용)} \\
...
% ===========================================================================
\section{스마트닝: Extractor 아키텍처}
% ===========================================================================

\subsection{ExtractorCore (Socratic 추출)}

SLM이 사용자 텍스트를 분석하여 민감 정보를 탐지한다. 소크라테스 추론을 사용하여 각 문장에 대해 3가지 질문을 던지고 자유 형식 SCREAMING\_CASE 카테고리를 유도한다:

\begin{verbatim}
Q1: 이 정보의 핵심 속성은 무엇인가?
Q2: 이 정보가 왜 민감한가?
Q3: 이 정보를 한 단어로 표현하면?
-> SCREAMING_SNAKE_CASE 카테고리
\end{verbatim}

\subsection{Critic (사후 검증, precision="high")}

\texttt{precision="high"}일 때, 두 번째 SLM이 ExtractorCore 결과를 검토하고 누락된 레코드를 보완한다. Critic은 비어있는 텍스트를 제외하고 항상 실행된다. 병합 시 다음 검증을 수행한다:

\begin{itemize}[nosep]
  \item 동일한 span + category 조합의 중복 제거
  \item 원본 텍스트에 존재하지 않는 span(할루시네이션) 자동 제거
  \item ExtractorCore와 동일한 검증 기준 (confidence $\geq$ 0.5, SCREAMING\_CASE 정규식)
\end{itemize}

\subsection{Judge (규칙 기반)}

Judge는 LLM 호출 없이 \texttt{is\_essential} 플래그를 기반으로 정책을 결정한다:

\begin{itemize}[nosep]
  \item \texttt{is\_essential > 0} $\rightarrow$ \texttt{route\_to\_local} (민감 정보가 질의 자체)
  \item 마스킹 가능 $\rightarrow$ \texttt{mask\_and\_send} (span을 플레이스홀더로 치환)
  \item 비민감 $\rightarrow$ \texttt{route\_to\_external} (변경 없이 전달)
\end{itemize}
...
\begin{enumerate}[nosep]
  ...
  \item 규칙 기반 Judge 구성(LLM 미사용)으로 월 \$0.075 수준의 운영 비용을 달성했다.
\end{enumerate}
